...
class LoanAgentExecutor(AgentExecutor):
    ...
    async def _process_request(
        ...
        async for event in self._run_agent(session_id, new_message):
            if event.is_final_response():
                parts = await convert_genai_parts_to_a2a(
            ...
            if not event.get_function_calls():
                ...
                await task_updater.update_status(
                    ...
                    message=task_updater.new_agent_message(
                        await convert_genai_parts_to_a2a(
                            event.content.parts
    ...
    async def execute(
        ...
        await self._process_request(
            types.UserContent(
                parts=await convert_a2a_parts_to_genai(context.message.parts),
            ),
            context.context_id,
            updater,
        )
...
async def convert_a2a_parts_to_genai(parts: list[Part]) -> list[types.Part]:
    """Convert a list of A2A Part types into a list of Google Gen AI Part types."""
    return [await convert_a2a_part_to_genai(part) for part in parts]


async def convert_a2a_part_to_genai(part: Part) -> types.Part:
...
async def convert_genai_parts_to_a2a(parts: list[types.Part]) -> list[Part]:
    """Convert a list of Google Gen AI Part types into a list of A2A Part types."""
    return [
        await convert_genai_part_to_a2a(part)
        for part in parts
        if (part.text or part.file_data or part.inline_data)
    ]


async def convert_genai_part_to_a2a(part: types.Part) -> Part:
